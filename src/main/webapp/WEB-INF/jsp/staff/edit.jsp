...
<!-- 引入js -->
<script type="text/javascript" language="javascript"
	src="assets/staff/edit.js"></script>
</head>
<body>
	<h1>修改员工信息</h1>
	<div class="container">
		<fieldset>
			<legend>修改一名员工</legend>
			<form action="staff/list" method="post">
				<button type="submit">返回表格</button>
			</form>
			<form class="staff-form" action="staff/edit" method="post" enctype="multipart/form-data" >
				<div>
					<label for="id"> 编号：</label> <input type="text" name="id" id="id"
						readonly="readonly" value="${staff.id }">
				</div>
				<div>
					<label for="staffId">员工号：</label> <input type="text" name="staffId"
						id="staffId" placeholder="输入商品号" value="${staff.staffId}">
				</div>

				<div>
					<label for="name">名字：</label> <input type="text" name="name"
						id="name" placeholder="输入商品名称" value="${staff.name}">
				</div>
				<div class="sex">
					<label>性别：</label>
					<div>
				...
				<%-- <div>
					<label for="birthday">出生日期：</label> <input type="text"
						name="birthday" id="birthday" value="${staff.birthday}"
						autocomplete="off">
				</div> --%>
				...
				<div>
					<label for="salary">工资：</label> <input type="text" name="salary"
						id="salary" placeholder="件/套/个..." value="${staff.salary}">
				</div>
				<div>
					<label for="departId">部门号：</label> <input type="text"
						name="departId" id="departId" placeholder="整数"
						value="${staff.departId}">
				</div>
				<div>
					<label for="addressId">地址号：</label> <input type="text"
						name="addressId" id="addressId" placeholder="整数"
						value="${staff.addressId}">
				</div>
				<div>
					<label for="picture-pic">上传图片：</label> <input type="file"
						name="picture-pic" id="picture-pic" autocomplete="off">
				</div>
